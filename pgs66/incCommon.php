	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'PersonnalFile' => array('Personnal File', 'This section register and contain all record from personnel. ', 'resources/table_icons/application_form_magnify.png', 'Misc.'),
			...
			'vendor' => array('Vendor & Subcontractor Register', 'Vendor list for organization operation. Detail of vendor and the associated vendor performance evaluation also available for reference. ', 'resources/table_icons/group.png', 'ASSET'),
			...
			'Inventory' => array('Inventory & Asset Register', 'Inventory and asset register shall consist of organization asset. this list of equipment must have dedicated maintenacne and inspection procedure. Parts and consumables shall not be registered under this item. ', 'resources/table_icons/change_password.png', 'ASSET'),
			...
			'MWOPlanned' => array('Planned Scheduled', 'Planned scheduled maintenance for asset. The expected record shall be the generated checklist of equipment inspection and testing. ', 'resources/table_icons/application_form_magnify.png', 'Misc.'),
			...
			'orders' => array('Request Order', 'Product and services request order for organization to aquire resources. Request order can be linked to the price quotation, Purchase order, Delivery Order and payment request. Sometime, the RO can also act as the market survey for the team to check the price quotation for the services.', 'resources/table_icons/cash_register.png', 'RESOURCES'),
			...
			'Item' => array('Item & Equipment', 'Item and equipment shall consist of all parts and items that are consumables. This indicates the inventory of the fast moving item in the organization.', 'resources/table_icons/barcode.png', 'RESOURCES'),
			...
			'transactions' => array('Transactions', 'This table indicates movement of the asset and inventory in the organization.', 'resources/table_icons/book_keeping.png', 'RESOURCES'),
			...
			'IMSDataAnalysis' => array('IMS Data Analysis', 'Collection of analysis of data to be kept on this table for reference. this is to prepare for quaterly meeting content. ', 'resources/table_icons/chart_stock.png', 'QHSE'),
			...
			'ContinualImprovement' => array('Continual Improvement Plan', 'CIPL table consist of CAPAR register. ', 'resources/table_icons/finance.png', 'QHSE'),
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			...
			'Competency' => "`Competency` LEFT JOIN `employees` as employees1 ON `employees1`.`EmployeeID`=`Competency`.`EmployeeID` LEFT JOIN `ProjectTeam` as ProjectTeam1 ON `ProjectTeam1`.`ProjectTeamID`=`Competency`.`ProjectTeamID` ",
			'Training' => "`Training` LEFT JOIN `employees` as employees1 ON `employees1`.`EmployeeID`=`Training`.`EmployeeID` LEFT JOIN `ProjectTeam` as ProjectTeam1 ON `ProjectTeam1`.`ProjectTeamID`=`Training`.`ProjectTeamID` ",
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				$resultLogin = sqlValue("select count(1) from membership_users_control where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0");
				$resultGroup = sqlValue("select groupID from membership_users_control where lcase(memberID)='$username'");
				$resultUserDashboardData = sqlValues("select c.compID, c.name compName, c.address, u.memberID, u.email, g.groupID, g.name role from membership_users_control u inner join membership_company_control c on u.compID = c.compID inner join membership_groups_control g on u.groupID = g.groupID and g.compID = c.compID where lcase(u.memberID)='$username'");

				if(sqlValue("select count(1) from membership_users_control where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")>0){
					$_SESSION['memberID']=$username;
					$_SESSION['token']=$password;
					$_SESSION['resultUserDashboardData']=$resultUserDashboardData;
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>userDashboard.php"><img src="../assets/images/puffer-logo.png" /></a>
			</div>
	...
	function showNotifications($msg = '', $class = '', $fadeout = true){
		...
		$notify_template_no_fadeout = '<div id="%%ID%%" class="alert alert-dismissable %%CLASS%%" style="display: none; padding-top: 6px; padding-bottom: 6px;position: relative;z-index: 2;">' .
					'<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>' .
					'%%MSG%%</div>' .
					'<script> jQuery(function(){ jQuery("#%%ID%%").show("slow"); }); </script>'."\n";
		$notify_template = '<div id="%%ID%%" class="alert %%CLASS%%" style="display: none; padding-top: 6px; padding-bottom: 6px;position: relative;z-index: 2;">%%MSG%%</div>' .
					'<script>' .
						'jQuery(function(){' .
	...
	*/
	...
	function NavMenus($options = array()){
		...
		for($i = 0; $i < count($menu); $i++){
			$menu_wrapper .= <<<EOT
				<li>
					<a href="#" class="dropdown-toggle" data-toggle="dropdown">{$table_group_name[$i]} </a>
...
EOT;
	...
	function quick_search_html($search_term, $label, $separate_dv = true){
		...
		<div class="input-group" id="quick-search">
			<input type="text" id="SearchString" name="SearchString" value="{$safe_search}" class="form-control quick-search" placeholder="{$safe_label}">
			<span class="input-group-btn d-inline-block">
				<button name="Search_x" value="1" id="Search" type="submit" onClick="{$reset_selection}" class="btn btn-outline-secondary" title="{$safe_label}"><i class="glyphicon glyphicon-search"></i></button>
				<button name="ClearQuickSearch" value="1" id="ClearQuickSearch" type="submit" onClick="\$j('#SearchString').val(''); {$reset_selection}" class="btn btn-outline-secondary" title="{$safe_clear_label}"><i class="glyphicon glyphicon-remove-circle"></i></button>
			</span>
		</div>
EOT;
		return $html;
