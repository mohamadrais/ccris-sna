	...
	*/
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('PG_SNA_rememberMe', $username.$password, time()+86400*30);
					}else{
						@setcookie('PG_SNA_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['PG_SNA_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['PG_SNA_rememberMe']);
...
EOT;
	...
	function dbCalendarEvent($title, $start, $end, $tableName, $pkValue, $review, $approval, $imscontrol, $operation) {
		...
		if(!isset($imscontrol)) $imscontrol = 1;
		$sql_operation = '';
		$values = [];
	...
	function getWeeklyHoursEngaged($memberID, $type = "default"){
		$currDay = sqlValue("SELECT WEEKDAY(CURDATE())");
		if($type == "default") {
			$selectB = "";
			$selectA = "";
			$selectM = "";
			...
			$selectB = ", b.memberID, b.name, sum(b.hours_per_day)";
			$selectA = ", a.memberID, a.name";
			$selectM = ", mu.`memberID`, e.`Name` as 'name'";
			$whereMemberID = "";
			$groupByB = "group by 2, 3";
			$groupByA = ", 3, 4";
			$groupByM = "group by 3, 4, 5";
			$orderByB = "order by 1 desc";
			$limitB = " limit 5";
		}
		...
		if ($currDay > 0){
			...
			for($x=1; $x <= $currDay; $x++) {
				...
				if ($x != $currDay) $sql .= "union all ";
			}
		}
		$sql .= " ) a group by 2 $groupByA";
		$sql .= " ) b $groupByB $orderByB $limitB";
