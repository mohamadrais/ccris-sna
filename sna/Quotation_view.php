...
// Download AppGini for free from https://bigprof.com/appgini/download/
	...
	$x->QueryFieldsTV = array(   
		...
		"CONCAT('$', FORMAT(`Quotation`.`fo_Price`, 2))" => "fo_Price",
		...
		"`Quotation`.`ot_Ref03`" => "ot_Ref03",
		"`Quotation`.`ot_Ref04`" => "ot_Ref04",
		"`Quotation`.`ot_Ref05`" => "ot_Ref05",
		"`Quotation`.`ot_Ref06`" => "ot_Ref06",
		"`Quotation`.`ot_Photo01`" => "ot_Photo01",
		"`Quotation`.`ot_Photo02`" => "ot_Photo02",
		"`Quotation`.`ot_Photo03`" => "ot_Photo03",
		...
		"if(`Quotation`.`ot_ap_filed`,date_format(`Quotation`.`ot_ap_filed`,'%m/%d/%Y %h:%i %p'),'')" => "ot_ap_filed",
		"if(`Quotation`.`ot_ap_last_modified`,date_format(`Quotation`.`ot_ap_last_modified`,'%m/%d/%Y %h:%i %p'),'')" => "ot_ap_last_modified"
	);
	...
	$x->SortFields = array(   
		...
		28 => 28,
		29 => '`Quotation`.`ot_ap_filed`',
		30 => '`Quotation`.`ot_ap_last_modified`'
	);
	...
	$x->QueryFieldsFilters = array(   
		...
		"`Quotation`.`ot_Ref03`" => "Reference_3",
		"`Quotation`.`ot_Ref04`" => "Reference_4",
		"`Quotation`.`ot_Ref05`" => "Reference_5",
		"`Quotation`.`ot_Ref06`" => "Reference_6",
	...
	if(strpos($x->HTML, '<!-- tv data below -->')){
		...
		if(isset($_REQUEST['Print_x']) && is_array($_REQUEST['record_selector'])){
			...
			if($QueryWhere != ''){
				$QueryWhere = 'where `Quotation`.`id` in ('.substr($QueryWhere, 0, -1).')';
		...
		$sumQuery = "select CONCAT('$', FORMAT(sum(`Quotation`.`fo_Price`), 2)) from {$x->QueryFrom} {$QueryWhere}";
		$res = sql($sumQuery, $eo);
		if($row = db_fetch_row($res)){
			$sumRow = '<tr class="success">';
			if(!isset($_REQUEST['Print_x'])) $sumRow .= '<td class="text-center"><strong>&sum;</strong></td>';
			$sumRow .= '<td class="Quotation-QuoID"></td>';
			...
			$sumRow .= '<td class="Quotation-ot_ap_filed"></td>';
			$sumRow .= '<td class="Quotation-ot_ap_last_modified"></td>';
			$sumRow .= '</tr>';
