<script>
	$j(function(){
		...
		var data = {
			...
			fo_BaseLocation: <?php echo json_encode(array('id' => $rdata['fo_BaseLocation'], 'value' => $rdata['fo_BaseLocation'], 'text' => $jdata['fo_BaseLocation'])); ?>,
		...
		/* saved value for fo_BaseLocation */
		cache.addCheck(function(u, d){
			if(u != 'ajax_combo.php') return false;
			if(d.t == tn && d.f == 'fo_BaseLocation' && d.id == data.fo_BaseLocation.id)
				return { results: [ data.fo_BaseLocation ], more: false, elapsed: 0.01 };
			return false;
		});
