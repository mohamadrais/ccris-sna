<?php
	...
	$lookups = array(   
		'OrgContentContext' => array(
			'assignToWO' => array(
				'parent_table' => 'WorkOrder',
				'parent_pk_field' => 'id',
				'parent_caption' => 'IF(CHAR_LENGTH(`WorkOrder`.`WONumber`) || CHAR_LENGTH(`WorkOrder`.`Task`), CONCAT_WS(\'\', `WorkOrder`.`WONumber`, \': \', `WorkOrder`.`Task`), \'\')',
				'parent_from' => "`WorkOrder` WHERE `fo_EmployeeID` = (select `employees`.`EmployeeID` from `employees` where `employees`.`memberID` = '" . makeSafe($memberInfo['username']) . "')",
				'filterers' => array(),
				'custom_query' => '',
	...
	if(count($wheres)){
		if ($table_name == 'WorkOrder' && $field_name == 'assignToWO'){
