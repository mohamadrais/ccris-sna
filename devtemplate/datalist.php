...
class DataList{
	...
	// handle user commands ...
		...
		elseif($insert_y != ''){
			if( $_REQUEST["ACID"] ) $_REQUEST["ACID"].= "_01";
			if( $_REQUEST["APID"] ) $_REQUEST["APID"].= "_01";
			if( $_REQUEST["AuditNo"] ) $_REQUEST["AuditNo"].= "_01";
			if( $_REQUEST["BaseLocation"] ) $_REQUEST["BaseLocation"].= "_01";
			if( $_REQUEST["batch_no"] ) $_REQUEST["batch_no"].= "_01";
			if( $_REQUEST["BwmID"] ) $_REQUEST["BwmID"].= "_01";
			if( $_REQUEST["CalibrationID"] ) $_REQUEST["CalibrationID"].= "_01";
			if( $_REQUEST["CampaignID"] ) $_REQUEST["CampaignID"].= "_01";
			if( $_REQUEST["CAPARno"] ) $_REQUEST["CAPARno"].= "_01";
			if( $_REQUEST["CategoryName"] ) $_REQUEST["CategoryName"].= "_01";
			if( $_REQUEST["ccpID"] ) $_REQUEST["ccpID"].= "_01";
			if( $_REQUEST["ClaimNo"] ) $_REQUEST["ClaimNo"].= "_01";
			if( $_REQUEST["ClientID"] ) $_REQUEST["ClientID"].= "_01";
			if( $_REQUEST["CompanyID"] ) $_REQUEST["CompanyID"].= "_01";
			if( $_REQUEST["CompID"] ) $_REQUEST["CompID"].= "_01";
			if( $_REQUEST["CompletionNo"] ) $_REQUEST["CompletionNo"].= "_01";
			if( $_REQUEST["CondBaseID"] ) $_REQUEST["CondBaseID"].= "_01";
			if( $_REQUEST["ContMonitNo"] ) $_REQUEST["ContMonitNo"].= "_01";
			if( $_REQUEST["CorrectiveID"] ) $_REQUEST["CorrectiveID"].= "_01";
			if( $_REQUEST["CRID"] ) $_REQUEST["CRID"].= "_01";
			if( $_REQUEST["DillID"] ) $_REQUEST["DillID"].= "_01";
			if( $_REQUEST["DocconNumber"] ) $_REQUEST["DocconNumber"].= "_01";
			if( $_REQUEST["DocumentName"] ) $_REQUEST["DocumentName"].= "_01";
			if( $_REQUEST["DOID"] ) $_REQUEST["DOID"].= "_01";
			if( $_REQUEST["EmpNo"] ) $_REQUEST["EmpNo"].= "_01";
			if( $_REQUEST["ENID"] ) $_REQUEST["ENID"].= "_01";
			if( $_REQUEST["ExecutionNo"] ) $_REQUEST["ExecutionNo"].= "_01";
			if( $_REQUEST["FileID"] ) $_REQUEST["FileID"].= "_01";
			if( $_REQUEST["InitiationNo"] ) $_REQUEST["InitiationNo"].= "_01";
			if( $_REQUEST["InqNumber"] ) $_REQUEST["InqNumber"].= "_01";
			if( $_REQUEST["InventoryID"] ) $_REQUEST["InventoryID"].= "_01";
			if( $_REQUEST["ItemID"] ) $_REQUEST["ItemID"].= "_01";
			if( $_REQUEST["LogisticID"] ) $_REQUEST["LogisticID"].= "_01";
			if( $_REQUEST["LogisticNumber"] ) $_REQUEST["LogisticNumber"].= "_01";
			if( $_REQUEST["LRNumber"] ) $_REQUEST["LRNumber"].= "_01";
			if( $_REQUEST["ManagingVendNumber"] ) $_REQUEST["ManagingVendNumber"].= "_01";
			if( $_REQUEST["MgtVstID"] ) $_REQUEST["MgtVstID"].= "_01";
			if( $_REQUEST["MWONumber"] ) $_REQUEST["MWONumber"].= "_01";
			if( $_REQUEST["OrderID"] ) $_REQUEST["OrderID"].= "_01";
			if( $_REQUEST["PlannedID"] ) $_REQUEST["PlannedID"].= "_01";
			if( $_REQUEST["PlanningNo"] ) $_REQUEST["PlanningNo"].= "_01";
			if( $_REQUEST["POID"] ) $_REQUEST["POID"].= "_01";
			if( $_REQUEST["PreventiveID"] ) $_REQUEST["PreventiveID"].= "_01";
			if( $_REQUEST["ProactiveID"] ) $_REQUEST["ProactiveID"].= "_01";
			if( $_REQUEST["ProgressReportId"] ) $_REQUEST["ProgressReportId"].= "_01";
			if( $_REQUEST["projectID"] ) $_REQUEST["projectID"].= "_01";
			if( $_REQUEST["QmID"] ) $_REQUEST["QmID"].= "_01";
			if( $_REQUEST["QuoID"] ) $_REQUEST["QuoID"].= "_01";
			if( $_REQUEST["ReactiveID"] ) $_REQUEST["ReactiveID"].= "_01";
			if( $_REQUEST["recID"] ) $_REQUEST["recID"].= "_01";
			if( $_REQUEST["RecNum"] ) $_REQUEST["RecNum"].= "_01";
			if( $_REQUEST["RecordID"] ) $_REQUEST["RecordID"].= "_01";
			if( $_REQUEST["RecordNumber"] ) $_REQUEST["RecordNumber"].= "_01";
			if( $_REQUEST["ResourcesID"] ) $_REQUEST["ResourcesID"].= "_01";
			if( $_REQUEST["RISKid"] ) $_REQUEST["RISKid"].= "_01";
			if( $_REQUEST["tbmID"] ) $_REQUEST["tbmID"].= "_01";
			if( $_REQUEST["TimesheetID"] ) $_REQUEST["TimesheetID"].= "_01";
			if( $_REQUEST["Title"] ) $_REQUEST["Title"].= "_01";
			if( $_REQUEST["TraningNo"] ) $_REQUEST["TraningNo"].= "_01";
			if( $_REQUEST["TransCode"] ) $_REQUEST["TransCode"].= "_01";
			if( $_REQUEST["VariationNo"] ) $_REQUEST["VariationNo"].= "_01";
			if( $_REQUEST["VendPerfNumber"] ) $_REQUEST["VendPerfNumber"].= "_01";
			if( $_REQUEST["WONumber"] ) $_REQUEST["WONumber"].= "_01";
	...
	// quick search and TV action buttons  
		if(!$this->HideTableView && !($dvprint_x && $this->AllowSelection && $SelectedID) && !$PrintDV){
			...
			if(count($selected_records_more)){
				$more_menu .= '<button type="button" class="btn btn-outline-secondary dropdown-toggle" data-toggle="dropdown" id="selected_records_more"><i class="glyphicon glyphicon-check"></i> ' . $Translation['More'] . ' </button>';
	...
	// display multiple printable detail views
		...
		$this->HTML .= "</div></div></div></form>";
		$this->HTML .= '</div><div class="col-xs-1 md-hidden lg-hidden"></div></div></div>';
					...
					<div class="row" id="toggle-columns">
						<div class="col-md-12">
							<div class="btn-group" style="width: 100%;">
								<button type="button" class="btn btn-outline-secondary" id="show-all-columns" style="width: 33.3%;"><i class="glyphicon glyphicon-check"></i> <?php echo $Translation['Reset Filters']; ?></button>
								<button type="button" class="btn btn-outline-secondary" id="hide-all-columns" style="width: 33.3%;"><i class="glyphicon glyphicon-unchecked"></i> <?php echo $Translation['hide all']; ?></button>
								<button type="button" class="btn btn-outline-secondary" id="toggle-columns-checks" style="width: 33.4%;"><i class="glyphicon glyphicon-random"></i> <?php echo $Translation['toggle']; ?></button>
							</div>
						</div>
						<div class="col-md-12"><button type="button" class="btn btn-outline-secondary btn-block" id="toggle-columns-collapser" data-toggle="collapse" data-target="#toggle-columns-container"><i class="glyphicon glyphicon-ok"></i> <?php echo $Translation['ok']; ?></button></div>
					</div>
				</div>
				<div class="clearfix"></div>
			</div> -->
				<ul class="dropdown-menu sm container" role="menu" id="toggle-columns-container">
					<div class="sort-header"><button type="button" class="btn btn-outline-secondary" id="show-all-columns"><i class="glyphicon glyphicon-check"></i> <?php echo $Translation['Reset Filters']; ?></button></div>
					<div class="sort-body" id="toggle-columns" style="max-height: 300px; overflow: scroll;">
						<div class="col-md-12" id="toggle-columns-collapser"></div>
					</div>
					<div class="sort-footer"><button type="button" class="btn btn-primary" data-toggle="collapse" data-target="#toggle-columns-container"><i class="glyphicon glyphicon-ok"></i> <?php echo $Translation['ok']; ?></button></div>
					<div class="clearfix"></div>
				</ul>
		<?php // } ?>
