...
<script>
	$j(function(){
		var tn = 'DailyProgressReport';

		/* data for selected record, or defaults if none is selected */
		var data = {
			DPRID: <?php echo json_encode(array('id' => $rdata['DPRID'], 'value' => $rdata['DPRID'], 'text' => $jdata['DPRID'])); ?>,
		...
		/* saved value for DPRID */
		cache.addCheck(function(u, d){
			if(u != 'ajax_combo.php') return false;
			if(d.t == tn && d.f == 'DPRID' && d.id == data.DPRID.id)
				return { results: [ data.DPRID ], more: false, elapsed: 0.01 };
			return false;
		});
