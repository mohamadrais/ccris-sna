	...
	*/
		...
		$all_tables = array(
			/* ['table_name' => [table props assoc array] */   
				'OrgContentContext' => array(
					'Caption' => 'Organization Content & Context',
				...
				'Inventory' => array(
					'Caption' => 'Inventory & Asset Register',
				...
				'MWOPlanned' => array(
					'Caption' => 'Planned Scheduled',
					'Description' => 'Planned scheduled maintenance for asset. The expected record shall be the generated checklist of equipment inspection and testing. ',
					'tableIcon' => 'resources/table_icons/application_form_magnify.png',
					'group' => $tg[7],
					'homepageShowCount' => 0
				),
				...
				'projects' => array(
					...
					'Description' => 'List of project in the organization',
					'tableIcon' => 'resources/table_icons/server_components.png',
					'group' => $tg[0],
					'homepageShowCount' => 1
				),
				'WorkLocation' => array(
					'Caption' => 'Work Site Location',
	...
	if(!function_exists('getTableList')){
		function getTableList($skip_authentication = false){
			$arrTables = array(   
				...
				'IMSDataAnalysis' => 'IMS Data Analysis',
				'Audit' => 'Management System Audit',
				'NonConformance' => 'IMS Non Conformance',
				'ContinualImprovement' => 'Continual Improvement Plan',
	...
	function sqlValues($statment){
		...
		while ($row = db_fetch_assoc($res)) {	
			$resultSet[$i]["compID"] = $row["compID"];
			$resultSet[$i]["compName"] = $row["compName"];
			$resultSet[$i]["address"] = $row["address"];
			$resultSet[$i]["memberID"] = $row["memberID"];
			$resultSet[$i]["email"] = $row["email"];
			$resultSet[$i]["groupID"] = $row["groupID"];
			$resultSet[$i]["role"] = $row["role"];
			$i++;
		}
